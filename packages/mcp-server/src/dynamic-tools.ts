...
export function dynamicTools(endpoints: Endpoint[]): Endpoint[] {
  ...
  const listEndpointsTool = {
    ...
    handler: async (client: Flowglad, args: Record<string, unknown> | undefined): Promise<ToolCallResult> => {
  ...
  const getEndpointTool = {
    ...
    handler: async (client: Flowglad, args: Record<string, unknown> | undefined) => {
      if (!args) {
