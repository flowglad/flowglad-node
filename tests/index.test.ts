...
import { APIPromise } from '@flowglad/node/core/api-promise';
...
describe('instantiate client', () => {
  ...
  describe('logging', () => {
    ...
    beforeEach(() => {
      process.env = { ...env };
      process.env['FLOWGLAD_LOG'] = undefined;
    });
    ...
    const forceAPIResponseForClient = async (client: Flowglad) => {
    ...
    test('debug logs happen with debug env var', async () => {
      ...
      const client = new Flowglad({ logger: logger, apiKey: 'My API Key' });
    ...
    test('warn when env var level is invalid', async () => {
      ...
      expect(warnMock).toHaveBeenCalledWith(
        'process.env[\'FLOWGLAD_LOG\'] was set to "not a log level", expected one of ["off","error","warn","info","debug"]',
      );
    });
  ...
  describe('baseUrl', () => {
    test('trailing slash', () => {
      const client = new Flowglad({ baseURL: 'http://localhost:5000/custom/path/', apiKey: 'My API Key' });
    ...
    afterEach(() => {
      process.env['FLOWGLAD_BASE_URL'] = undefined;
    });

    test('explicit option', () => {
      const client = new Flowglad({ baseURL: 'https://example.com', apiKey: 'My API Key' });
    ...
    test('in request options overridden by client options', () => {
      const client = new Flowglad({ apiKey: 'My API Key', baseURL: 'http://localhost:5000/client' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
    ...
    test('in request options overridden by env variable', () => {
      process.env['FLOWGLAD_BASE_URL'] = 'http://localhost:5000/env';
      const client = new Flowglad({ apiKey: 'My API Key' });
      expect(client.buildURL('/foo', null, 'http://localhost:5000/option')).toEqual(
  ...
  test('maxRetries option is correctly set', () => {
    const client = new Flowglad({ maxRetries: 4, apiKey: 'My API Key' });
    expect(client.maxRetries).toEqual(4);

    // default
    const client2 = new Flowglad({ apiKey: 'My API Key' });
...
describe('retries', () => {
  ...
  test('retry on 429 with retry-after', async () => {
    ...
    const client = new Flowglad({ apiKey: 'My API Key', fetch: testFetch });
